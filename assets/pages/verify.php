...
<div class="col-span-3 lg:col-span-1 shadow-2xl w-full flex flex-col items-center justify-center p-5 bg-white rounded-none lg:rounded-r-3xl">
    ...
    <form autocomplete="off" action="../php/action.php" method="post" class="w-full flex flex-col items-center">
        ...
        <div class="space-y-3 w-3/5 mb-4">
            <p class="font-semibold">Verify Your Email Address</p>
            <input type="text" placeholder="Enter your Email Address" name="email"  id="thisEmail" class="focus:outline-none border-3 border-gray-400 focus:border-green-500 rounded-lg p-2 w-full <?php if(isset($_SESSION['sendError'])) echo 'border-red-500 bg-red-200'; ?>" required> <!--border-red-500 bg-red-200-->
        ...
        <button type="submit" name="verify" class="rounded-2xl flex flex-row items-center justify-center space-x-2 py-3 w-3/5 bg-green-800 hover:bg-green-600 text-white font-bold mt-4 transform hover:scale-105 transition duration-500">
          <p>Send Code</p>
      </button>

      <div class="space-y-3 w-3/5 mb-10 mt-4">
        <a href="signup.php"><p class="text-green-900 italic hover:underline">Back to Login</p></a>
      </div>
