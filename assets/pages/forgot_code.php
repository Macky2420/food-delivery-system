...
<div class="col-span-3 lg:col-span-1 shadow-2xl w-full flex flex-col items-center justify-center p-5 bg-white rounded-none lg:rounded-r-3xl">
    ...
    <form autocomplete="off" action="../php/action.php" method="post" class="w-full flex flex-col items-center">
        ...
        <div class="space-y-3 w-3/5 mb-4">
            <p class="font-semibold">Please Enter the Code we Sent</p>
            <input type="text" placeholder="000-000" name="for_code" id="codeId" class="focus:outline-none border-3 border-gray-400 focus:border-green-500 rounded-lg p-2 w-full <?php if(isset($_SESSION['coderror'])) echo 'border-red-500 bg-red-200'; ?>" required> <!--border-red-500 bg-red-200-->
            
            <?php if(isset($_SESSION['codeError'])): ?>
                <p class="text-xs text-red-500"><?php echo $_SESSION['codeError']; ?></p> 
            <?php endif; ?>
          
        </div>
                <!-- Next it can be Submit and remove the a tag -->
        <button type="submit" name="forgot_code" class="rounded-2xl flex flex-row items-center justify-center space-x-2 py-3 w-3/5 bg-green-800 hover:bg-green-600 text-white font-bold mt-4 transform hover:scale-105 transition duration-500">
            <p>Verify</p>
      </button>

      <div class="space-y-3 w-3/5 mb-10 mt-4">
        <a href="../php/forgot_resend.php"><p class="text-green-900 hover:underline">Resend Code</p></a>
      </div>
...
<script>
    ...
    codeId.addEventListener('input', () => {
    codeId.classList.remove('border-red-500', 'bg-red-200');
    codeId.classList.add('border-green-500');

    const errorElement = document.querySelector('#codeId + p.text-xs.text-red-500');
    if (errorElement) {
        errorElement.remove();
        <?php unset($_SESSION['codeError']); ?>
    }
    });
</script>
